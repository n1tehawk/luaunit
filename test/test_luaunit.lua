...
TestLuaUnitUtilities = { __class__ = 'TestLuaUnitUtilities' }
    ...
    function TestLuaUnitUtilities:test_sortedNextWorks()
        ...
        local tested = {}
        for _, vv in lu.private.sortedPairs(t1) do table.insert(tested, vv) end
    ...
    function TestLuaUnitUtilities:test_IsFunction()
        ...
        lu.assertNotNil( lu.LuaUnit.asFunction( function() end ) )
...
TestLuaUnitErrorMsg = { __class__ = 'TestLuaUnitErrorMsg' }
    ...
    function TestLuaUnitErrorMsg:test_assertNotIsThread()
        assertFailureContains( 'Not expected: a thread type, actual: value thread:', lu.assertNotIsThread, coroutine.create( function(v) return v+1 end ) )
    end 
    ...
    function TestLuaUnitErrorMsg:test_assertErrorMsg()
        assertFailureEquals('No error generated when calling function but expected error: "bla bla bla"' , lu.assertErrorMsgEquals, 'bla bla bla', function( v ) return v+1 end, 3 )
        assertFailureEquals('No error generated when calling function but expected error containing: "bla bla bla"' , lu.assertErrorMsgContains, 'bla bla bla', function( v ) return v+1 end, 3 )
        assertFailureEquals('No error generated when calling function but expected error matching: "bla bla bla"' , lu.assertErrorMsgMatches, 'bla bla bla', function( v ) return v+1 end, 3 )
        ...
        assertFailureEquals('Error message does not match: "bla bla bla"\nError message received: "toto xxx"\n' , lu.assertErrorMsgMatches, 'bla bla bla', function() error('toto xxx',2) end, 3 )
...
TestLuaUnitResults = { __class__ = 'TestLuaUnitResults' }
    ...
    function TestLuaUnitResults:test_statusLine()
        -- full success
        local r = {runCount=5, duration=0.17, passedCount=5, notPassedCount=0,
                   failureCount=0, errorCount=0, skippedCount=0, nonSelectedCount=0}
        ...
        r = {runCount=5, duration=0.17, passedCount=4, notPassedCount=1,
             failureCount=1, errorCount=0, skippedCount=0, nonSelectedCount=0}
        lu.assertEquals( lu.LuaUnit.statusLine(r), 'Ran 5 tests in 0.170 seconds, 4 successes, 1 failure')

        -- 1 error, no failure displayed
        r = {runCount=5, duration=0.17, passedCount=4, notPassedCount=1,
             failureCount=0, errorCount=1, skippedCount=0, nonSelectedCount=0}
        lu.assertEquals( lu.LuaUnit.statusLine(r), 'Ran 5 tests in 0.170 seconds, 4 successes, 1 error')

        -- 1 error, 1 failure 
        r = {runCount=5, duration=0.17, passedCount=3, notPassedCount=2,
             failureCount=1, errorCount=1, skippedCount=0, nonSelectedCount=0}
        ...
        r = {runCount=5, duration=0.17, passedCount=3, notPassedCount=2,
             failureCount=1, errorCount=1, skippedCount=0, nonSelectedCount=1}
        ...
        r = {runCount=5, duration=0.17, passedCount=5, notPassedCount=0,
             failureCount=0, errorCount=0, skippedCount=0, nonSelectedCount=1}
        lu.assertEquals( lu.LuaUnit.statusLine(r), 'Ran 5 tests in 0.170 seconds, 5 successes, 0 failures, 1 non-selected')

        -- mix including a skipped test
        r = {runCount=5, duration=0.17, passedCount=1, notPassedCount=2,
             failureCount=1, errorCount=1, skippedCount=1, nonSelectedCount=1}
        lu.assertEquals( lu.LuaUnit.statusLine(r), 'Ran 5 tests in 0.170 seconds, 1 success, 1 skipped, 1 failure, 1 error, 1 non-selected')
    end
    ...
    function TestLuaUnitResults:test_nodeStatus()
        ...
        es:skipped( 'msgToto3', 'stackTraceToto3' )
        lu.assertEquals( es.status, lu.NodeStatus.SKIPPED )
        lu.assertTrue( es:isNotPassed() )
        lu.assertFalse( es:isFailure() )
        lu.assertFalse( es:isError() )
        lu.assertTrue( es:isSkipped() )
        lu.assertEquals( es.msg, 'msgToto3' )
        lu.assertNil( es.stackTrace )
        lu.assertStrMatches( es:statusXML(), ".*<!%-%- <skipped>.*</skipped> %-%->.*" )
    ...
    function TestLuaUnitResults:test_runSuiteOk()
        ...
        lu.assertEquals( runner.result.testCount, 9 )
        lu.assertEquals( runner.result.runCount, 9 )
        lu.assertEquals( runner.result.passedCount, 7 )
        lu.assertEquals( runner.result.skippedCount, 2 )
